...
{
    private static DbContextOptions<SettingsDbContext> InMemoryOptions() =>
    ...
    public async Task Existence_Check_With_Canonical_Key_Finds_Previously_Seeded_Flag()
    {
        ...
        await using (var seed = new SettingsDbContext(options, new TenantProvider()))
        {
        ...
        await using var db = new SettingsDbContext(options, new TenantProvider());
