...
Status: **plan / proposal — grilled & scoped to a Phase-0 spike** (no code landed)
Date: 2026-09-16 (updated after grill-me session — see `brainstorms/prefab-ui-portals.md`)
...
**new side project** (root-level `portals/` folder, same repo) that consumes
...
| # | Branch | Decision |
|---|---|---|
| Q1 | Repo placement | Same repo, root-level `portals/`, registered in the AppHost via `AddPythonApp` — only existing-file touch is one additive AppHost block |
...
3. Register in `Program.cs` mirroring the `families` block (app lives at the
   repo root, so the relative path is `../../../portals`):
   ```csharp
   var portals = builder.AddPythonApp("portals", "..\\..\\..\\portals", "app.py")
...
└── portals  (NEW — Python, Prefab UI, uv)
...
```
School-Collab/
└── portals/                      (NEW — root-level side project, Python, Prefab UI, uv)
    ├── pyproject.toml / uv.lock  (deps: prefab-ui, httpx)
    ├── app.py                    (entrypoint launched solely by AddPythonApp)
...
- Scaffold root-level `portals/` with a one-page Prefab app; register with
...
1. **Spike-success criteria for the ward MVP go/no-go** — define after Phase 0,
   ...
   being evaluated).
2. **Teacher review portal, OIDC, and CI for the `portals/` folder** — deferred
