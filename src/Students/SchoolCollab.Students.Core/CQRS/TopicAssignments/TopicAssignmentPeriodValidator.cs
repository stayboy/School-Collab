...
public static class TopicAssignmentPeriodValidator
{
    ...
    /// an AcademicYear or a Term/Semester within the tenant's active academic year.
    /// Null = year-spanning date-based delivery (back-compat).
    /// </summary>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (period.PeriodType != requiredType.Value)
            throw new TopicAssignmentPeriodException(
                $"A {group.Span} activity group topic requires a {requiredType} period, but '{periodId}' is a {period.PeriodType}.",
                periodId);
    }
}
