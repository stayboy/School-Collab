            ...
            CancellationToken ct) =>
        {
            try
            {
                await handler.HandleAsync(new UpdateStudent(id, req.FirstName, req.LastName,
                    req.DateOfBirth, req.GenderCodedValueId, req.TitleCodedValueId), ct);
...
internal record UpdateStudentRequest(Guid? TitleCodedValueId, string FirstName, string LastName, DateOnly? DateOfBirth, Guid? GenderCodedValueId);
