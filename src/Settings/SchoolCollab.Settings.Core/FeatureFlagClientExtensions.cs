...
public static class ConfigFeatureFlagClientExtensions
{
    ...
    /// resolved from the Settings bounded context (FeatureFlag aggregate) rather
    /// than <see cref="IConfiguration"/> alone.
    /// <para>
    /// Registers: <see cref="HybridCache"/> (L1 in-proc + L2 Redis), a named
    /// <see cref="System.Net.Http.HttpClient"/> pointed at the Aspire-discovered
    /// <c>settings-api</c> service (the unified replacement for the legacy
    /// <c>config-api</c> after the Settings context merge — see
    /// documents/solution/settings-context-merge-spec.md §8), an
    /// <see cref="IFeatureFlagResolver"/>, and replaces the default
    /// <see cref="IFeatureFlagService"/> registration with
    ...
    /// </para>
    /// <para>
    /// Cold-start fallback: when the Settings API is unreachable the client reads
    /// <c>FeatureFlags:FEATURE:*</c> from <c>IConfiguration</c>, so consumers keep
    /// working without the Settings service running (preserves dev out-of-the-box).
    /// </para>
    /// </summary>
        ...
        IConfiguration configuration)
    {
        ...
        // Aspire service discovery URL. The unified Settings API host is registered
        // in the AppHost as `settings-api` (replaces the legacy `config-api`).
        // `https+http://` is Aspire's HTTPS-with-HTTP-fallback scheme.
        services.AddHttpClient(ConfigFeatureFlagService.HttpClientName, c =>
            c.BaseAddress = new Uri("https+http://settings-api"));
