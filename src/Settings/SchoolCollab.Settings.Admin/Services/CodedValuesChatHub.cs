...
namespace SchoolCollab.Settings.Admin.Services;

/// <summary>
/// Scoped bridge that lets the inline <c>CodedValuesChat</c> on the landing page
/// mirror its conversation into the <see cref="CodedValuesChat"/> instance
/// living inside the side-drawer panel. Both surfaces share the same scoped
/// instance per render circuit, so the inline chat pushes each new
/// <see cref="CodedValuesChat.ChatMessageItem"/> via
/// <see cref="AddMessage"/> and the drawer chat reads via <see cref="Messages"/>.
///
...
/// obvious.
/// </summary>
public class CodedValuesChatHub
{
    private readonly List<CodedValuesChat.ChatMessageItem> _messages = [];
    ...
    /// </summary>
    public IReadOnlyList<CodedValuesChat.ChatMessageItem> Messages => _messages;
    ...
    /// inline chat. Reset to <see cref="CodedValuesChat.ChatStreamingState"/>
    /// defaults when streaming ends or the conversation is cleared.
    /// </summary>
    public CodedValuesChat.ChatStreamingState StreamingState { get; private set; } =
    ...
    /// </summary>
    public void AddMessage(CodedValuesChat.ChatMessageItem message)
    {
    ...
    /// </summary>
    public void SetStreamingState(CodedValuesChat.ChatStreamingState state)
    {
    ...
    public void Clear()
    {
        ...
        _messages.Clear();
        StreamingState = new CodedValuesChat.ChatStreamingState(false, string.Empty, null);
        Changed?.Invoke();
    }
}
